...
const MarbleCalculator = ({ setPriceData, prodPri, productId }) => {
  ...
  const handleSubmit = () => {
    const parsedProductPrice = parseInt(prodPri);
    ...
    const obj = {
      ...
      runningfoot,
      parsedProductPrice,
      prodPrice: parsedProductPrice,
    };
    setPriceData(obj);

    const userId = userDetails?._id || "random";
    ...
    if (existingItemIndex !== -1) {
      // Update existing record if the product ID already exists
      ...
      // Create a new cart item object if the product ID doesn't exist
      const newItem = {
        userId: userId,
        productId: productId,
        ...obj,
    ...
    setShowCartModal(true); // Show the cart modal after successful addition
  };
